...
import { TrendingUp, TrendingDown, ArrowUpDown, Copy, Network, Loader2, CheckCircle, DollarSign } from 'lucide-react';
...
export default function USDTWalletPage() {
  ...
  const { wallet, loading } = useUSDTWallet();
  const { trasactions, loading: transactionsLoading } = useTransactions();
  const [copiedAddress, setCopiedAddress] = useState<string | null>(null);

  const usdtTransactions = trasactions.filter(tx => tx.market?.quote_unit === 'usdt' || tx.market?.base_unit === 'usdt');

  const networks = [
    { id: 'trc20', name: 'TRC-20', network: 'Tron', color: 'bg-red-500', textColor: 'text-red-400' },
    { id: 'erc20', name: 'ERC-20', network: 'Ethereum', color: 'bg-electric-blue', textColor: 'text-electric-blue' },
    { id: 'bep20', name: 'BEP-20', network: 'BSC', color: 'bg-yellow-500', textColor: 'text-yellow-400' }
  ];

  const getAddressByNetwork = useCallback((networkId: string) => {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-2xl font-bold text-soft-white mb-2">USDT Wallet</h1>
        ...
        {loading ? (
          ...
          <>
            <div className="text-center mb-6">
              ...
              <h2 className="text-3xl font-bold text-soft-white">{parseFloat(wallet?.balance || '0').toFixed(2)} USDT</h2>
      ...
      <div className="space-y-6">
        <h3 className="text-lg font-semibold text-soft-white">Network Addresses</h3>
        
        {networks.map((network) => {
          const addressData = getAddressByNetwork(network.id);
          
          return (
            <div key={network.id} className="bg-dark-gray p-6 rounded-xl border border-medium-gray">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center space-x-3">
                  <div className={`h-10 w-10 ${network.color} bg-opacity-20 rounded-full flex items-center justify-center`}>
                    <span className={`${network.textColor} text-sm font-bold`}>{network.name[0]}</span>
                  </div>
                  <div>
                    <h4 className="font-semibold text-soft-white">USDT {network.name}</h4>
                    <p className="text-sm text-gray-400">{network.network} Network</p>
                  </div>
                </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  {loading ? (
                    <div className="flex items-center justify-center py-4">
                      <Loader2 className={`h-5 w-5 animate-spin ${network.textColor}`} />
                    </div>
                  ) : addressData?.status === 'pending' ? (
                    <div className="p-4 bg-medium-gray rounded-lg">
                      <p className="text-sm text-gray-400">Address generating...</p>
                    </div>
                  ) : addressData?.address ? (
                    <div className="flex items-center justify-between p-4 bg-medium-gray rounded-lg">
                      <span className="text-sm font-mono text-soft-white break-all mr-4">{addressData.address}</span>
                      <button
                        onClick={() => copyToClipboard(addressData.address, network.id)}
                        className={`p-2 ${network.textColor} hover:bg-opacity-20 hover:${network.color.replace('bg-', 'bg-')} rounded transition-colors flex-shrink-0`}
                      >
                ...
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="text-soft-white">
                    {network.id === 'trc20' ? 'Low' : network.id === 'bep20' ? 'Medium' : 'High'}
                  </span>
                </div>
              </div>
            </div>
      ...
      <div className="bg-dark-gray p-6 rounded-xl border border-medium-gray">
        <h3 className="text-lg font-semibold text-soft-white mb-4">Network Information</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
          ...
          <div className="p-4 bg-medium-gray rounded-lg">
            <h4 className="font-medium text-electric-blue mb-2">ERC-20 (Ethereum)</h4>
      ...
      <div className="bg-dark-gray rounded-xl border border-medium-gray">
        <div className="p-6 border-b border-medium-gray">
          <h3 className="text-lg font-semibold text-soft-white">USDT Transaction History</h3>
        </div>
        <div className="p-6">
          {transactionsLoading ? (
            ...
            <div className="space-y-4">
              {usdtTransactions.length > 0 ? (
                usdtTransactions.map((tx) => (
                  <div key={tx.id} className="flex items-center justify-between p-4 bg-medium-gray rounded-lg">
                    <div className="flex items-center space-x-4">
                      ...
                      <div>
                        <p className="font-medium text-soft-white">{tx.side} USDT</p>
                        <p className="text-sm text-gray-400">{new Date(tx.created_at).toLocaleString()}</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="font-medium text-soft-white">{tx.volume} USDT</p>
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-sm text-gray-500">Your USDT transactions will appear here</p>
                </div>
              )}
            </div>
          )}
